      ...
      <div className={`flex items-center  md:justify-between ${!inputOpen && 'justify-between' } `}>
        <div className='flex items-center gap-2' >
          <button onClick={open} className="lg:hidden">
            ...
            {/* <li onClick={() => { setIsOpenn(false) }}>X</li> */}
            {hamburgerOpen ? (
                  // If hamburgerOpen is true, show "X"
                  <li className="text-xl border-b w-full list-none" onClick={toggleHamburgerOpen}>X</li>
                ) : (
                  // If hamburgerOpen is false, show the three lines
...
(
  ...
  <div className='w-[100px]  lg:w-[150px] md:ml-[20px]  '>
            ...
            <Link href={'/'}>
              ...
              <h1 onClick={()=> console.log('clicked')} className='lg:ml-[150px] cursor-pointer font-bold text-[20px]'>Eccommerce</h1>
            </Link>
  ...
  <FontAwesomeIcon
    ...
    className={`text-gray-500 text-xl block md:hidden  cursor-pointer absolute right-2 ${!inputOpen && 'right-[-20]'} top-1/2 transform -translate-y-1/2   ${!inputOpen && "translate-x-12"}
